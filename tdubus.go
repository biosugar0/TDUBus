...
func getTimeholiday(data *goquery.Selection) (Schedule, Schedule) {
	...
	d.Each(func(n int, s *goquery.Selection) {
		if n > 0 {
			hour := s.Find("th").Text()
			mbase := s.Find("td")
			ret1.Kumagaya[hour] = makeSchedule(mbase.Slice(0, 1))
			ret1.Kitasakado[hour] = makeSchedule(mbase.Slice(1, 2))
			ret1.Takasaka[hour] = makeSchedule(mbase.Slice(2, 3))
			ret2.Takasaka[hour] = makeSchedule(mbase.Slice(3, 4))
			ret2.Kitasakado[hour] = makeSchedule(mbase.Slice(4, 5))
			ret2.Kumagaya[hour] = makeSchedule(mbase.Slice(5, 6))
		}
	})
	return ret1, ret2
}
